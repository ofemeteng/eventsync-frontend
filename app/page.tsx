...
function ChatInterface() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('https://d341-194-110-85-167.ngrok-free.app/chat', {
        method: 'POST',
  ...
  return (
    <div className={`flex items-center justify-center min-h-screen p-4 relative overflow-hidden ${
      ...
      <Card className={`w-full max-w-4xl backdrop-blur-sm shadow-xl rounded-3xl overflow-hidden z-10 border-4 ${
        theme === 'dark'
        ...
        <CardHeader className={`relative ${
          ...
          <div className="flex justify-center mt-4 space-x-4">
            <div className={`flex items-center rounded-full px-3 py-1 ${
              theme === 'dark' ? 'bg-indigo-800/60' : 'bg-indigo-100 shadow-md'
            }`}>
        ...
        <CardContent className="p-6">
          <div className={`rounded-2xl shadow-inner p-4 h-[400px] overflow-y-auto mb-4 relative border-2 ${
            theme === 'dark'
            ...
            {messages.length === 0 ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                <Calendar className={`h-16 w-16 mb-4 animate-pulse ${
              ...
              messages.map((message, index) => (
                ...
                >
                  <span
                    className={`inline-block px-4 py-2 rounded-2xl shadow-sm max-w-[80%] ${
                      message.sender === 'user'
                        ? theme === 'dark'
            ...
            {isTyping && (
              <div className="text-left mb-3">
                <span className={`inline-block px-4 py-2 rounded-2xl shadow-sm ${
                  theme === 'dark'
          ...
          <div className="flex space-x-2">
            <Textarea
              ...
              className={`flex-grow border-2 focus:ring focus:ring-opacity-50 rounded-xl resize-none shadow-sm transition-all duration-200 ${
                theme === 'dark'
            ...
            <Button 
              onClick={sendMessage} 
              className={`self-end font-semibold py-2 px-4 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 ${
                theme === 'dark'
